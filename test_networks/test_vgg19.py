...
def preprocess_image():
    # find smaller dimension and resize it to loadSize (while keeping aspect ratio)
...
#     out = image.crop(input, w1, h1, w1 + oW, h1 + oW) - - center
#     return out

def load_pretrained_model():

    model = os.path.join(pycaffe_dir, "models", "VGG_ILSVRC_19_layers.caffemodel")
    ...
    # classifier.
    model_definitions = os.path.join(pycaffe_dir, "models", "VGG_ILSVRC_19_layers_reduced_deploy.prototxt")
    classifier = caffe.Classifier(model_definitions, model,
                                  image_dims='256,256', mean=bgr_means,
                                  raw_scale=255.0,
                                  channel_swap='2,1,0') # rgb image assumed, convert to bgr
...
def main(argv):
    ...
    parser.add_argument(
        "--model_def",
        default=os.path.join(pycaffe_dir, "vgg_face_caffe",
